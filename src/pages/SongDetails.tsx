...
const SongDetails = () => {
  ...
  const seedTracks = useMemo(() => (track ? [track] : []), [track]);
  const { query: tasteQuery, loading: loadingTaste } = useTasteProfile(seedTracks);
  ...
  const moreLikeThis = useMemo(() => related.slice(0, 6), [related]);

  /* ---------- Recommended for you (taste profile) ---------- */
  useEffect(() => {
    if (!track || !tasteQuery) return;
    let cancelled = false;
    setLoadingRecommended(true);
    (async () => {
      try {
        const res = await fetch(
          `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/youtube-search?q=${encodeURIComponent(tasteQuery)}`,
          { headers: await getFunctionAuthHeaders() }
        );
        if (!res.ok) throw new Error('failed');
        const data = await res.json();
        ...
        const moreLikeThisIds = new Set(moreLikeThis.map((t) => t.id));
        ...
        for (const item of Array.isArray(data) ? data : []) {
          ...
          if (moreLikeThisIds.has(item.id)) continue;
          seenIds.add(item.id);
          filtered.push(item);
        }

        setRecommendedTracks(filtered.slice(0, 12));
      } catch {
        if (!cancelled) setRecommendedTracks([]);
      } finally {
        if (!cancelled) setLoadingRecommended(false);
      }
    })();
    return () => { cancelled = true; };
  }, [track?.id, tasteQuery, moreLikeThis]);
  ...
  const recommended = recommendedTracks;
          ...
          {loadingTrack ? (
            ...
            <>
              ...
              <section className="mt-10">
                ...
                {loadingRecommended || loadingTaste ? (
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                    {[...Array(6)].map((_, i) => (
                      <Skeleton key={i} className="aspect-square rounded-[2rem]" />
                    ))}
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                    {recommended.map((t) => (
                      <TrackCard
                        ...
                        isCurrent={currentTrack?.id === t.id}
                        isPlaying={isPlaying}
                        onPlay={(sel) => handlePlayTrack(sel, recommended)}
                        onAddToQueue={handleAddToQueue}
                        isFavorite={isFavorite(t.id)}
                        onToggleFavorite={toggleFavorite}
                      />
                  ...
                  <p className="text-sm text-muted-foreground">No recommendations found right now.</p>
                )}
              </section>
            </>
          )}
